...
public class ActivityLifecycleCallbacksImpl implements Application.ActivityLifecycleCallbacks {
    ...
    public void onActivityCreated(Activity activity, Bundle savedInstanceState) {
        if (AutoSizeConfig.getInstance().isCustomFragment()) {
            // TODO: 2018/11/13 @Ding
            //这里会在activity被创建后调用，如果是fragmentActivity，就会去创建fragment的生命周期方法，然后回掉
        ...
        if (mAutoAdaptStrategy != null) {
            Log.e(TAG, "onActivityCreated: activity创建时调用一次applyadapt");
    ...
    public void onActivityStarted(Activity activity) {
        if (mAutoAdaptStrategy != null) {
            Log.e(TAG, "onActivityStarted: activitystarted时调用一次applyadapt");
            mAutoAdaptStrategy.applyAdapt(activity, activity);
        }
    }
